...
class NotificacionWidget extends StatefulWidget {
  ...
  const NotificacionWidget({super.key, required this.notificacion, required this.indice});

  @override
  State<NotificacionWidget> createState() => _NotificacionWidgetState();


  const factory NotificacionWidget.agregaNotificacion({required Notificacion notificacion, required int indice}) =
      _NuevaNotificacionModelWidget;
}

class _NotificacionWidgetState extends State<NotificacionWidget> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    opacidad =
        Tween(begin: 0.1, end: 1.0).animate(CurvedAnimation(parent: controller, curve: const Interval(0, 0.25, curve: Curves.easeOut)));

    opacidadOut =
        Tween(begin: 1.0, end: 0.0).animate(CurvedAnimation(parent: controller, curve: const Interval(0.9, 1.0, curve: Curves.easeOut)));

    bajar =
        Tween(begin: -60.0, end: 10.0).animate(CurvedAnimation(parent: controller, curve: const Interval(0, 0.25, curve: Curves.easeOut)));

    subir =
        Tween(begin: 20.0, end: -60.0).animate(CurvedAnimation(parent: controller, curve: const Interval(0.8, 1.0, curve: Curves.easeOut)));

    progress =
        Tween(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: controller, curve: const Interval(0.65, 0.75, curve: Curves.easeOut)));
    ...
    controller.addListener(() {
      ...
      if (controller.status == AnimationStatus.completed) {
        context.read<NotificacionesBloc>().add(OnEliminaNotificacionEvent(indice: widget.indice, notificacion: widget.notificacion));
      }
    });
          ...
          child: Opacity(
            ...
            child: Transform.translate(
                ...
                child: Opacity(
                  ...
                  child: Container(
                    key: _key,
                    constraints: BoxConstraints(maxWidth: context.ancho * 100, minWidth: 0),
                      ...
                      children: [
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              Icon(
                                widget.notificacion.icono ?? _determinarIcono(widget.notificacion.tipoNotificacion),
                                color: _determinarColor(widget.notificacion.tipoNotificacion),
                              ),
                              const SizedBox(width: 5),
                              Flexible(
                                child: TextModelWidget.titulo(
                                  texto: widget.notificacion.descripcion,
                                  colorTexto: _determinarColor(widget.notificacion.tipoNotificacion),
                                ),
                              ),
                            ],
                          ),
...
class _NuevaNotificacionModelWidget extends NotificacionWidget {
  const _NuevaNotificacionModelWidget({required Notificacion notificacion, required int indice})
